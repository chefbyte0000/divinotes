...
function svgDark(body, hx = 16, hy = 16, w = 32, h = 32) {
	const s = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">${body}</svg>`;
	const filled = s.replace(/DOT/g, D.dot).replace(/RING/g, D.ring).replace(/MUT/g, D.mut).replace(/DEST/g, D.dest);
	return { u: enc(filled), hx, hy };
}
...
for (const [k, v] of Object.entries(p)) {
	...
	out += `\t\t--divi-cursor-${k.replace(/([A-Z])/g, "-$1").toLowerCase().replace(/^-/, "")}: ${v.light};\n`;
}
out += `\t}\n\n\thtml.dark {\n`;
for (const [k, v] of Object.entries(p)) {
	if (k === "none") continue;
	out += `\t\t--divi-cursor-${k.replace(/([A-Z])/g, "-$1").toLowerCase().replace(/^-/, "")}: ${v.dark};\n`;
}
out += `\t}\n`;

const rules = [];

function add(sel, varName) {
	const key = varName.replace(/-([a-z])/g, (_, c) => c.toUpperCase());
	const cssKey = varName;
	rules.push(`\t${sel} { cursor: var(--divi-cursor-${cssKey}); }`);
}
...
pointerish.forEach((s) => rules.push(`\t${s} { cursor: var(--divi-cursor-pointer); }`));

rules.push(
	`\tinput[type="checkbox"]:not(:disabled), input[type="radio"]:not(:disabled) { cursor: var(--divi-cursor-pointer); }`
);
...
textish.forEach((s) => rules.push(`\t${s} { cursor: var(--divi-cursor-text); }`));

rules.push(
	`\t:disabled, [aria-disabled="true"], [data-disabled], .pointer-events-none { cursor: var(--divi-cursor-not-allowed); }`
);
rules.push(`\tbutton:not(:disabled) [contenteditable="true"] { cursor: var(--divi-cursor-text); }`);

// Disabled controls: clearer not-allowed (override pointer from parent if any)
rules.push(
	`\tbutton:disabled, [role="button"][aria-disabled="true"], input:disabled, textarea:disabled, select:disabled { cursor: var(--divi-cursor-not-allowed); }`
);
...
for (const [sel, key] of tw) {
	if (key === "none") {
		...
		rules.push(`\t${sel} { cursor: var(--divi-cursor-${key.replace(/-/g, "-")}); }`);
	}
}
